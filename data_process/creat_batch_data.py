...
def ch_train_get_batch(title_len=52, content_len=300, batch_size=128):
    ...
    p = Pool()
    X_title = np.asarray(list(p.map(pad_X52, train_title)))
    X_content = np.asarray(list(p.map(pad_X300, train_content)))
    p.close()
    p.join()
    X = np.hstack([X_title, X_content])
...
def ch_test_get_batch(title_len=52, content_len=300, batch_size=128):
    ...
    p = Pool()
    X_title = np.asarray(list(p.map(pad_X52, eval_title)))
    X_content = np.asarray(list(p.map(pad_X300, eval_content)))
    p.close()
    p.join()
    X = np.hstack([X_title, X_content])
    sample_num = len(X)
