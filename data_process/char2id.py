...
def test_char2id():
    ...
    # 转为 id 形式
    p = Pool()
    eval_title = np.asarray(list(p.map(get_id4chars, df_eval.char_title.values)))
    np.save('../data/ch_eval_title.npy', eval_title)
    eval_content = np.asarray(list(p.map(get_id4chars, df_eval.char_content.values)))
...
def train_char2id():
    ...
    df_trains = pd.read_csv('../raw_data/question_train_set.txt', sep='\t', usecols=[0, 1, 3],
                            iterator = True, 
                            names=['question_id', 'char_title', 'char_content'], dtype={'question_id': object})
    ...
    na_title_indexs = [328877, 422123, 633584, 768738, 818616, 876828]    
    ...
    train_content = np.asarray(list(p.map(get_id4chars, df_train.char_content.values)))
